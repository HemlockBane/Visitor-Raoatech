...
export class VisitorProvider {
  ...
  fire_employee = firebase.database().ref('/employee')
  ...
  getEmployee(phone) {
    var promise = new Promise((resolve, reject) => {
      ...
      this.fire_employee.child(this.afauth.auth.currentUser.uid).orderByChild('phone').once('value', snapshot => {
        let data = snapshot.val
        let employeeArray = []

        for (let k in employeeArray){
          employeeArray.push(data[k])
        }
